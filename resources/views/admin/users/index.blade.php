    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ...
                @if ($admin->role === 'admin')
                    <span class="text-base font-normal text-gray-500">(Hanya
                        Penyiar{{ $admin->studio_id ? ' di Studio ' . $admin->studio->nama : '' }})</span>
                @endif
            </h2>
                    ...
                    @if (session('success'))
                        <div class="mb-4 flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg shadow-sm"
                            ...
                            <button @click="show = false"
                                class="ml-auto text-green-600 hover:text-green-800">&times;</button>
                        </div>
                    @endif
                    @if (session('error'))
                        <div class="mb-4 flex items-center gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg shadow-sm"
                            ...
                            <button @click="show = false"
                                class="ml-auto text-red-600 hover:text-red-800">&times;</button>
                        </div>
                    @endif

                    {{-- Form Filter Kondisional --}}
                    @if ($admin->role !== 'admin') {{-- Hanya Katim yang melihat filter --}}
                        <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                            <form method="GET" action="{{ route('admin.users.index') }}"
                                ...
                                {{-- Tombol Filter & Reset --}}
                                <div class="flex gap-2 md:col-start-3">
                                    <button type="submit"
